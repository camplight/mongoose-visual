...
function abstractModel(obj) {
  ...
  var collectionProtoype = obj.schema.paths;
  var items = new Array();
...
module.exports = visual = function(args) {
  if (args[0] == 'docs') {
    process.on('modelsLoaded', function() {
      ...
      fs.writeFileSync(process.cwd() + "/models.json", writeReady, "utf8");
      // Save HTML
      var HTML = HTMLString + '</body></html>';
      fs.writeFileSync(process.cwd() + "/models.html", HTML, "utf8");
    });
    var dir = args[1];
    fs.readdir(process.cwd() + dir, function(error, files) {
      if (error) {
        ...
        files.forEach(function(file) {
          var modelName = file.replace(path.extname(file), '');
          var model = require(process.cwd() + dir + modelName);
